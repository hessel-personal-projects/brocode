...
**Tech Stack:** Next.js 16 (App Router, React 19, TypeScript), Prisma 7 + Postgres, **Supabase** (Postgres + Storage — replaces the spec's Neon + Cloudflare R2, per user decision), `@supabase/supabase-js` v2, Resend v6, Zod v4, Tailwind CSS v4, pnpm, Vitest (unit + integration), Playwright (E2E). Local dev/test runs the full Supabase stack via the Supabase CLI (`supabase start`). (Versions reflect the latest majors installed at execution time — the plan text below was authored against Next 15 / Prisma 6 / Zod 3 / Resend 4; the APIs used are compatible, except `next lint` was replaced by `eslint .` on Next 16.)
